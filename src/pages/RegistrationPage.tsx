...
export default function LoginPage() {
  ...
  const leftPanel = (
    <div
      className="hidden md:flex flex-1 w-full h-full relative bg-cover bg-center" // Changed lg:flex to md:flex
    ...
    >
      ...
      <div className="absolute top-6 left-6 z-20 flex items-center justify-between w-[80%]">
        <h2 className="text-white text-2xl font-semibold">Selected Works</h2>
      ...
      <div className="absolute bottom-6 left-6 z-20 flex items-center space-x-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex flex-col text-white">
            ...
            <span className="text-xs opacity-75">UI &amp; Illustration</span>
          </div>
        </div>
  ...
  const rightPanel = (
    <div className="relative flex flex-col justify-center items-center w-full h-full"> {/* Added wrapper for centering and relative positioning of lang selector */}
      {/* Sélecteur de langue en haut à droite */}
      <div className="absolute top-6 right-6 z-10"> {/* Ensure lang selector is positioned relative to this new wrapper */}
        <button
      ...
      {/* Formulaire central */}
      <AuthFormCard> {/* AuthFormCard provides its own padding and max-width */}
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-1">Hi Designer</h1>
        <p className="text-gray-500 mb-6 text-sm sm:text-base">Welcome to UISOCIAL</p>
  ...
  return (
    <div className="flex items-center justify-center w-full h-screen bg-gray-100 p-4">
      <AuthLayout
        ...
        className="relative w-full max-w-6xl h-full bg-white rounded-3xl overflow-hidden shadow-xl" // Applied card styles here
      />
    </div>
  );
}
