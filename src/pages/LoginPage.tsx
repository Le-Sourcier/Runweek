  ...
  return (
    <div className="min-h-screen font-sans flex flex-col md:flex-row w-full">
      ...
      <div className="hidden md:flex md:w-1/2 bg-gray-50 items-center justify-center p-12 border-r border-gray-200 transition-opacity duration-700 ease-in-out">
        ...
        <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" className="w-3/4 max-w-lg h-auto transition-opacity duration-1000 ease-in-out opacity-100"> {/* Added transition and opacity */}
          <defs>
          ...
          <path fill="url(#grad1)" d="M76.4,-71.3C96.4,-59.4,108.2,-36.9,110.1,-13.6C112,9.7,103.9,33.8,88.4,49.2C72.9,64.6,49.9,71.4,28.1,74.4C6.2,77.3,-14.5,76.5,-34.9,69.1C-55.3,61.7,-75.4,47.8,-85.6,29.7C-95.8,11.6,-96.1,-10.7,-87.8,-29.9C-79.5,-49.1,-62.6,-65.2,-43.7,-73.3C-24.8,-81.4,-3.9,-81.5,15.9,-79C35.7,-76.5,56.4,-83.3,76.4,-71.3Z" transform="translate(100 100)" />
      ...
      <div className="w-full md:w-1/2 flex flex-col items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 p-4 sm:p-8 lg:p-12">
        <div className="w-full max-w-md p-10 space-y-8 bg-white rounded-xl shadow-2xl"> {/* Adjusted space-y, shadow */}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="relative" style={{ minHeight: '160px' }}> {/* Increased min-height slightly */}
              {/* Email Step */}
              <div className={`absolute w-full transform transition-all duration-300 ease-in-out ${currentStep === 'email' ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-5 pointer-events-none h-0'}`}>
                ...
                <div className="pt-6"> {/* Spacing for Next button */}
                  <button
                    type="button"
              ...
              <div className={`absolute w-full transform transition-all duration-300 ease-in-out ${currentStep === 'password' ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5 pointer-events-none h-0'}`}>
                <div className="mb-3">
                  <button type="button" onClick={handleBackStep} className="flex items-center text-sm text-blue-600 hover:text-blue-500 font-medium focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-blue-500 rounded-sm transform transition-opacity duration-150 ease-in-out hover:opacity-80 active:opacity-70">
                    <ArrowLeft className="h-4 w-4 mr-1" /> Back to email
                  </button>
                  {emailValue && <p className="text-sm text-gray-600 mt-1 truncate" title={emailValue}>Logging in as: {emailValue}</p>}
                </div>
                <div>
                ...
                <div className="pt-6"> {/* Spacing for Login button */}
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="group relative w-full flex justify-center items-center py-3 px-4 border border-transparent text-base font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:bg-blue-500 transform transition-transform duration-150 ease-in-out hover:scale-105 active:scale-95"
        ...
        <div className="mt-8 text-center"> {/* Ensure this still has appropriate spacing relative to card's space-y */}
