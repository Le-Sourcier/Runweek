...
async function askAI(userMessage, userContext, messagesHistory = []) {
  ...
  try {
    // Construire le tableau des messages pour OpenAI
    const messages = [
      {
        role: "system",
        content: `Tu es un coach personnel bienveillant et expert. Analyse le contexte fourni sur l'utilisateur (profil, activités récentes) pour donner des conseils personnalisés, encourageants et actionnables. Réponds ensuite à ses questions en maintenant le contexte de la conversation. Voici le contexte utilisateur : ${contextString}`,
      },
      ...messagesHistory,
      {
        role: "user",
